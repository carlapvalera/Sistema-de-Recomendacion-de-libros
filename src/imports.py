...
def remove_noise(texts):
    texts = [
            [word.lower() for word in doc if word.isalpha()] for doc in texts]
    return texts
...
def remove_stopwords(texts):
    ...
    return [[word for word in txt if word not in stoplist] for txt in texts]
...
def normalize_text(texts):
    ...
    txts = tokenization(txts_lower_case)

    #Remove noise
    txts = remove_noise(txts)
    # remove stopwords
    txts = remove_stopwords(txts)
    #morphological_reduction
...
def update_dictionary(texts):
    global dictionary
    # Create a dictionary from the stemmed tokens
    dictionary = corpora.Dictionary(texts)

def generate_bow(texts):
    update_dictionary(texts)
    # Create a bag-of-words model for each book, using the previously generated dictionary
    bows = [dictionary.doc2bow(t) for t in texts]
    return bows

def Dataframe(texts):
    bows = generate_bow(texts)
